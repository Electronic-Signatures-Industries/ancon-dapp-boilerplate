...
export class IPFSManager {
  ...
  async setCurrentNode(cid: string, key: string = 'self') {
    const res = await this.client.name.publish(cid, { key })
    return res
  }

  async getCurrentNode(key: string = 'self') {
    try {
      let query = ''
   ...
   */
  async addVideoObject(did: DID, payload: File, previousNode?: any) {
    let temp: string
    let content: Buffer
    if (payload instanceof File) {
      ...
      throw new Error('addVideoObject: must be a file object')
    }
    temp = temp.replace('0x', '')
    // sign the payload as dag-cbor
    //* display on grid

    const video = await this.client.add({
      path: payload.name,
      content,
    })
    const metadata = {
