  ...
  async createPadesNode(files?: File[]) {
    ...
    try {
      ...
      for (let index = 0; index < this.files.length; index++) {
        ...
        // @ts-ignore
        const resp2 = await sc.signXades(
          pin,
          ethers.utils.base64.encode(new Uint8Array(Buffer.from(xml).buffer))
        );        
        debugger
