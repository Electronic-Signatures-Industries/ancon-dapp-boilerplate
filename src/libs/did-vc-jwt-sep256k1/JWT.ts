...
export async function verifyJWT(
  ...
  const signer: PublicKey = await verifier(data, signature, authenticators);
  ...
  if (signer) {
    ...
    if (payload.nbf) {
      ...
      throw new Error(
        `JWT not valid yet (issued in the future) iat: ${payload.iat}`
      );
    }
    ...
    if (payload.aud) {
      if (isDIDOrMNID(payload.aud)) {
        ...
        if (aud !== normalizeDID(payload.aud)) {
          throw new Error(
            `JWT audience does not match your DID: aud: ${payload.aud} !== yours: ${aud}`
          );
        }
      } else {
        ...
        if (payload.aud !== options.callbackUrl) {
          throw new Error(
            `JWT audience does not match the callback url: aud: ${payload.aud} !== url: ${options.callbackUrl}`
          );
        }
      }
    }
...
export async function createJWT(
  payload: any,
  { issuer, signer, alg, expiresIn }: any,
  ...
  const signature: string = await signer(signingInput);
...
export async function resolveAuthenticator(
  ...
  const authenticationKeys: boolean | string[] = auth
    ...
    : true;
  const authenticators: PublicKey[] = (
    doc.publicKey || []
  ).filter(({ type, id }) =>
    types.find(
      supported =>
